...
export class WuiCarouselWrapper extends LitElement {
  ...
  @property() enableCounter: any;
  @property() enableArrows: any;
  ...
  firstUpdated(): void {
    ...
    this?.shadowRoot
      ?.querySelector('ul')
      ?.addEventListener('touchstart', (ev: TouchEvent) => this.__touchstart(ev));
    this?.shadowRoot
      ?.querySelector('ul')
      ?.addEventListener('touchend', (ev: TouchEvent) => this.__touchend(ev));
  }
  ...
  __setAnimationCssClass() {
    if (this.currentIndex === this.prevIndex) {
      return;
    }
    const oldCarousel = this?.shadowRoot?.querySelector('#oldCarousel');
    oldCarousel?.classList.add(
      this.currentIndex < this.prevIndex
        ? 'wui-old-carousel-slide-out'
        : 'wui-old-carousel-slide-in',
    );
    const newCarousel = this?.shadowRoot?.querySelector('#newCarousel');
    newCarousel?.classList.add(
      this.currentIndex < this.prevIndex
        ? 'wui-new-carousel-slide-out'
        : 'wui-new-carousel-slide-in',
    );
  ...
  __resetAnimation() {
    this?.shadowRoot?.querySelector('#oldCarousel')?.removeAttribute('class');
    this?.shadowRoot?.querySelector('#newCarousel')?.removeAttribute('class');
  }

  disconnectedCallback() {
    this?.shadowRoot
      ?.querySelector('ul')
      ?.removeEventListener('touchstart', this.__touchstart.bind(this));
    this?.shadowRoot
      ?.querySelector('ul')
      ?.removeEventListener('touchend', this.__touchend.bind(this));
    ...
    return html`
      ${this.carouselItems.length > 0
        ? html`
          ...
          `
        : null}
      ${this.enableCounter && this.carouselItems.length > 0
        ? html`
  ...
  __getArrowsTemplate(): TemplateResult {
    return html`
      <button
        id="left"
        name="left"
        class="wui-carousel-button"
        @click="${(event: { target: { name: string } }) => this.__onItemChange(event.target.name)}"
      >
        <svg viewBox="0 0 16 16" preserveAspectRatio="xMidYMid meet" focusable="false">
          <g viewBox="0 0 16 16">
            <path
              d="M11.5,3.591L9.969,2L4.711,7.469C4.572,7.615,4.5,7.808,4.5,8s0.072,0.385,0.211,0.531L9.969,14l1.53-1.591L7.262,8L11.5,3.591z"
            ></path>
          </g>
        </svg>
      </button>
      ...
      >
        <svg viewBox="0 0 16 16" preserveAspectRatio="xMidYMid meet" focusable="false">
          <g viewBox="0 0 16 16">
            <path
              d="M11.288,7.469L6.03,2L4.5,3.591L8.739,8L4.5,12.409L6.03,14l5.258-5.469C11.43,8.385,11.5,8.192,11.5,8S11.43,7.615,11.288,7.469z"
            ></path>
          </g>
        </svg>
      </button>
    ...
    switch (direction) {
      case 'left': {
        if (this.currentIndex > 0 && this.currentIndex < this.carouselItems.length) {
          ...
          this.dispatchEvent(
            new CustomEvent('selected-item', { detail: { selectedIndex: currentIndex } }),
          );
        }
        break;
      }
